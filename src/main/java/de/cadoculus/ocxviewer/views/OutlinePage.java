...
import static atlantafx.base.util.BBCodeParser.createFormattedText;
...
public abstract class OutlinePage extends StackPane implements Page {
    ...
    protected void createPageLayout() {
        ...
        userContent.setMinWidth(Page.MIN_WIDTH - OUTLINE_WIDTH - 100);
    ...
    protected void addPageHeader() {
        ...
        var titleLbl = new Label(this.getName());
        titleLbl.getStyleClass().add(Styles.TITLE_2);

        userContent.getChildren().add(titleLbl);
    }
    ...
    protected void addFormattedText(String text, boolean handleUrl) {
        userContent.getChildren().add(createFormattedText(text));
    }

    protected void addSection(String title, Node content) {
        var titleIcon = new FontIcon(MaterialDesignA.AB_TESTING);
