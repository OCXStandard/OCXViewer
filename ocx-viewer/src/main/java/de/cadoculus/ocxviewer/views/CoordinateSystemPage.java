...
import org.ocx_schema.v3x.QuantityT;
 ...
 */
public class CoordinateSystemPage extends AbstractDataViewSubPage<org.ocx_schema.v3x.CoordinateSystem> implements Page {
    ...
    public CoordinateSystemPage(org.ocx_schema.v3x.CoordinateSystem coosys, Page parent) {
    ...
    private Tab createTab(String title, org.ocx_schema.v3x.RefPlanesT refPlanesT) {

        var repfplanesTab = new Tab(title);

        var vbox = new VBox();

        if (refPlanesT instanceof org.ocx_schema.v3x.XRefPlanes xRefPlanes) {
        ...
        ObservableList<org.ocx_schema.v3x.RefPlaneT> refplanes = FXCollections.observableArrayList();
        if (refPlanesT != null && refPlanesT.getRefPlanes() != null && !refPlanesT.getRefPlanes().isEmpty()) {
            refplanes.addAll(refPlanesT.getRefPlanes());
        }

        var tableColumn1 = new TableColumn<org.ocx_schema.v3x.RefPlaneT, String>("ID");
        tableColumn1.setCellValueFactory(
                c -> new SimpleStringProperty(c.getValue().getId())
        );

        var tableColumn2 = new TableColumn<org.ocx_schema.v3x.RefPlaneT, String>("GUID");
        tableColumn2.setCellValueFactory(
                c -> new SimpleStringProperty(c.getValue().getGUIDRef())
        );

        var tableColumn3 = new TableColumn<org.ocx_schema.v3x.RefPlaneT, String>("Name");
        tableColumn3.setCellValueFactory(
                c -> new SimpleStringProperty(c.getValue().getName())
        );

        var tableColumn4 = new TableColumn<org.ocx_schema.v3x.RefPlaneT, Boolean>("Display");
        tableColumn4.setCellValueFactory(
                c -> new SimpleBooleanProperty(c.getValue().isDisplayGrid())
        );

        var tableColumn5 = new TableColumn<org.ocx_schema.v3x.RefPlaneT, QuantityT>("Location");
