...
/**
 * An open cone or truncated-cone surface (no bottom cap) defined by an origin
 * (base centre), a tip (apex centre), a base radius and an optional tip radius.
 * <p>
 * When {@code tipRadius} is 0 the result is a sharp cone; when it is positive
 * the result is a frustum.  In both cases only the lateral surface is rendered.
 * <p>
 * The mesh is built in local space with the axis along +Z (base at z=0,
 * tip at z=height) and then rotated so that +Z aligns with the vector
 * (tip − origin).
 */
public class Cone3D extends Group implements InformationProvider {

    private static final int DIVISIONS = 72;
     ...
     * @param originX     world X of the cone base centre
     * @param originY     world Y of the cone base centre
     * @param originZ     world Z of the cone base centre
     * @param tipX        world X of the cone tip centre
     * @param tipY        world Y of the cone tip centre
     * @param tipZ        world Z of the cone tip centre
     * @param baseRadius  radius at the base (origin end)
     * @param tipRadius   radius at the tip end; 0 for a sharp cone
    ...
    public Cone3D(String name,
                  ...
                  double baseRadius, double tipRadius,
        ...
        Vector3d axisVec = new Vector3d(tipX - originX, tipY - originY, tipZ - originZ);
        double height = axisVec.length();
        if (height < 1e-9) {
            return; // degenerate — skip geometry
        }
        axisVec.normalize();

        MeshView meshView = new MeshView(buildLateralMesh(baseRadius, tipRadius, height));
        meshView.setMaterial(material);
        meshView.setCullFace(CullFace.BACK);

        applyAxisRotation(meshView, axisVec);
    ...
    /**
     * Builds a TriangleMesh for the open lateral surface of a cone/frustum.
     * The base ring sits at z=0 with {@code baseRadius}; the tip ring sits at
     * z=height with {@code tipRadius} (0 for a sharp cone — all tip vertices
     * collapse to the apex point).
     */
    private static TriangleMesh buildLateralMesh(double baseRadius, double tipRadius, double height) {
        int n = DIVISIONS;

        // 2*n points: [0..n-1] base ring, [n..2n-1] tip ring
        float[] pts = new float[n * 2 * 3];
        for (int i = 0; i < n; i++) {
            double angle = 2.0 * Math.PI * i / n;
            float cosA = (float) Math.cos(angle);
            float sinA = (float) Math.sin(angle);
            pts[i * 3]           = (float) (baseRadius * cosA);
            pts[i * 3 + 1]       = (float) (baseRadius * sinA);
            pts[i * 3 + 2]       = 0f;
            pts[(n + i) * 3]     = (float) (tipRadius * cosA);
            pts[(n + i) * 3 + 1] = (float) (tipRadius * sinA);
            pts[(n + i) * 3 + 2] = (float) height;
        }

        // (n+1) columns × 2 rows (base u,0 / tip u,1)
