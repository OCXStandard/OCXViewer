...
public class NurbsCone implements NurbsSurface{
	
	NurbsSurface ns=null;
	
	public NurbsCone(double minorRadius, double majorRadius)
	{
//		NurbsCurve circle = NurbsCreator.createArc(new Origin3D(new Point3d(new double[]{0,0,0})),1,2, 2);
		NurbsCurve circle = NurbsCreator.createSegment(new Point3f(0,0,0), new Point3f(1,0,1));
//		NurbsCurve circle=NurbsCreator.createFullCircle(new Origin3D(new Point3d(new double[]{0,0,0})), (float)minorRadius, NurbsCreator.FC_QUAD9);
		ns=NurbsCreator.createRevolvedSurface(new Axis3D(new Point3d(new double[]{0,Float.MIN_VALUE,0}),new Vector3d(1,0,0)), circle, 2*Math.PI);
//		ns=NurbsCreator.createSwungSurface( line,circle);
	}
